...
class STGNNInferenceService:
    ...
    def __init__(self, data_path: str | None = None):
        ...
        default_candidates = [
            project_root / "ml" / "hybrid_geoai_predictions.geojson",
            project_root / "ml" / "test_gnn_results_v2.geojson",
            project_root / "ml" / "test_gnn_results.geojson",
            project_root / "frontend" / "public" / "data" / "map_data_predictions.geojson",
        ]
        self.data_path = Path(data_path) if data_path else next((p for p in default_candidates if p.exists()), default_candidates[0])
