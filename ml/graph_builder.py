        ...
        for i in range(total_nodes):
            for j, dist in zip(indices[i], distances[i]):
                if i != j:
                    ...
                    # Nodes with similar elevation are more likely to be in the same aquifer layer
                    elev_diff = abs(elevations[i] - elevations[j])
                    w_elev = np.exp(-elev_diff / 50.0) # Decay constant of 50m
                    
                    # 3. LULC Similarity (Surface condition proxy)
                    w_lulc = 1.0 if lulc[i] == lulc[j] else 0.5
                    
                    # 4. Aquifer/Geology Similarity (Subsurface connectivity)
                    # Nodes in the same aquifer share hydraulic properties
                    w_aquifer = 1.2 if aquifers[i] == aquifers[j] and aquifers[i] != 0 else 0.8
